...
const text = [
  ...
  const form = document.querySelector(".lorem-form") // the entire btn inpput

  const amount = document.getElementById("amount") // can only input no.s
  const result = document.querySelector(".lorem-text") // where text will be
  ...
  form.addEventListener("submit", function(e){
    e.preventDefault() 
    const value = parseInt(amount.value) // to make the no. inputted, outputted as a no. too
    console.log(value)
    ...
    result.innerHTML = `<p class="result">${text[random]}</p>`
        ...
        let textTemp = text.slice(0, value) //0 means - the start of array to whatever value 
        
        textTemp = textTemp.map(function(item){ // for every value create a new para el
        return `<p class="result">${item}</p>`
        })
   
    result.innerHTML = textTemp.join("")
    }
